...
def main() -> None:
    ...
    run_all = args.all or not any(
        [args.reddit, args.trends, args.social, args.resale, args.sentiment,
    ...
    if want_insights:
        if args.offline_insights or not config.OPENAI_API_KEY:
            ...
            print(f"[7/7] Insights: offline rule engine ({why})"); rules.run()
        else:
            from solesight.insights import llm
            print("[7/7] Insights: OpenAI"); llm.run()
